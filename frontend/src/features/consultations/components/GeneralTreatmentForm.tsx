...
const INITIAL_FORM_DATA: TreatmentFormData = {
  ...
  mode_of_transaction: 'walk-in' as 'walk-in' | 'visited' | 'referral',
  referred_from: '',
  referred_to: '',
  pertinent_history: '',
...
export default function GeneralTreatmentForm({
  ...
  const [newBiteData, setNewBiteData] = useState({
    new_bite_date: '',
    new_bite_place: '',
    new_exposure_type: 'bite' as 'bite' | 'scratch' | 'lick' | 'other',
    new_severity: 'moderate' as 'minor' | 'moderate' | 'severe',
    new_animal_type: 'dog' as 'dog' | 'cat' | 'other',
    new_animal_status: 'unknown' as 'alive' | 'dead' | 'unknown' | 'rabid',
  ...
  useEffect(() => {
    ...
    if (initialIncident) {
      setNewBiteData({
        new_bite_date: initialIncident.bite_date ? String(initialIncident.bite_date).split('T')[0] : new Date().toISOString().split('T')[0],
    ...
    api.get(`/treatment-records/patient/${patientId}${biteId ? `?bite_id=${biteId}` : ''}`)
      .then((response) => {
        ...
        const activeInc = response.data?.active_bite_incident || initialIncident;
        if (activeInc) {
          setNewBiteData({
            new_bite_date: activeInc.bite_date ? String(activeInc.bite_date).split('T')[0] : new Date().toISOString().split('T')[0],
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        {(isReturningNewBite
          ...
          <div style={{
            ...
            {/* Header */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, borderBottom: '1px solid #dbeafe', paddingBottom: '12px' }}>
            ...
            {episodeHistory.length > 0 && (
              <div style={{ marginBottom: 16, padding: '12px 14px', borderRadius: 8, background: '#ffffff', border: '1px solid #bfdbfe' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: '#1e3a8a', textTransform: 'uppercase', letterSpacing: '0.4px' }}>
                    Prior Episode &amp; Vaccination History
                  </div>
                  <span style={{ fontSize: 11, color: '#047857', fontWeight: 600, background: '#dcfce7', padding: '2px 8px', borderRadius: 12 }}>
                    ✓ Verified on File
                  </span>
                </div>
                {episodeHistory.slice(0, 3).map((episode) => (
                  <div key={episode.bite_id} style={{ display: 'flex', justifyContent: 'space-between', gap: 12, fontSize: 12, color: '#334155', padding: '3px 0' }}>
                    <span>Episode {episode.episode_number} · Bite Date: <strong>{episode.bite_date || 'Not recorded'}</strong></span>
                    <span style={{ fontWeight: 600, color: '#1e40af' }}>
                      {episode.plan?.plan_type ? episode.plan.plan_type.replaceAll('_', ' ').toUpperCase() : 'Prior Regimen'} · Completed Doses: {episode.completed_dose_days?.length ? episode.completed_dose_days.map((day: number) => `Day ${day}`).join(', ') : 'none'}
                    </span>
                  </div>
                ))}
                <div style={{ marginTop: 8, paddingTop: 8, borderTop: '1px dashed #e2e8f0', fontSize: 11, color: '#475569', lineHeight: 1.4 }}>
                  💡 <strong>WHO / DOH Protocol Guide:</strong> If completed PEP &gt;3 months ago → <strong>2-Dose Booster (Day 0 &amp; Day 3)</strong>. If within 3 months → <strong>1-Dose Booster (Day 0 only)</strong>. If incomplete/unverified → restart Full PEP.
                </div>
              </div>
            )}

            {/* 2. New Bite Incident Exposure Details (Editable) */}
            <div style={{ marginBottom: 16, padding: '14px', borderRadius: 8, background: '#ffffff', border: '1px solid #bfdbfe' }}>
              <div style={{ fontSize: 12.5, fontWeight: 700, color: '#1e3a8a', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: 12 }}>
                New Bite Incident &amp; Exposure Details
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 12 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 4 }}>Date of New Exposure</label>
                  <input
                    type="date"
                    value={newBiteData.new_bite_date}
                    onChange={(e) => setNewBiteData(prev => ({ ...prev, new_bite_date: e.target.value }))}
                    disabled={isFormDisabled}
                    style={{ width: '100%', padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: 6, fontSize: 12.5 }}
                  />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 4 }}>Place of Incident</label>
                  <input
                    type="text"
                    value={newBiteData.new_bite_place}
                    onChange={(e) => setNewBiteData(prev => ({ ...prev, new_bite_place: e.target.value }))}
                ...
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 4 }}>Exposure Severity / Category</label>
                  <select
                    value={newBiteData.new_severity}
                    onChange={(e: any) => setNewBiteData(prev => ({ ...prev, new_severity: e.target.value }))}
                    disabled={isFormDisabled}
                    style={{ width: '100%', padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: 6, fontSize: 12.5 }}
                  >
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 12 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 4 }}>Nature of Exposure</label>
                  <select
                    value={newBiteData.new_exposure_type}
                    onChange={(e: any) => setNewBiteData(prev => ({ ...prev, new_exposure_type: e.target.value }))}
                    disabled={isFormDisabled}
                    style={{ width: '100%', padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: 6, fontSize: 12.5 }}
                  >
                ...
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 4 }}>Animal Involved</label>
                  <select
                    value={newBiteData.new_animal_type}
                    onChange={(e: any) => setNewBiteData(prev => ({ ...prev, new_animal_type: e.target.value }))}
                    disabled={isFormDisabled}
                    style={{ width: '100%', padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: 6, fontSize: 12.5 }}
                  >
                ...
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 4 }}>Animal Status</label>
                  <select
                    value={newBiteData.new_animal_status}
                    onChange={(e: any) => setNewBiteData(prev => ({ ...prev, new_animal_status: e.target.value }))}
                    disabled={isFormDisabled}
                    style={{ width: '100%', padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: 6, fontSize: 12.5 }}
                  >
                ...
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 4 }}>Washed with Soap &amp; Water?</label>
                  <select
                    value={newBiteData.new_site_washed ? 'yes' : 'no'}
                    onChange={(e) => setNewBiteData(prev => ({ ...prev, new_site_washed: e.target.value === 'yes' }))}
                    disabled={isFormDisabled}
                    style={{ width: '100%', padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: 6, fontSize: 12.5 }}
                  >
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 12 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 4 }}>Anatomical Body Part Exposed</label>
                  <input
                    type="text"
                    value={newBiteData.new_body_part}
                    onChange={(e) => setNewBiteData(prev => ({ ...prev, new_body_part: e.target.value }))}
                ...
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 4 }}>Wound Description &amp; Clinical Findings</label>
                  <input
                    type="text"
                    value={newBiteData.new_wound_description}
                    onChange={(e) => setNewBiteData(prev => ({ ...prev, new_wound_description: e.target.value }))}
            ...
            <div style={{ marginBottom: 16, padding: '12px 14px', borderRadius: 8, background: '#f8fafc', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <strong style={{ fontSize: 14, color: '#1e293b' }}>{formData.temperature ? `${formData.temperature} °C` : 'Not measured'}</strong>
                      {formData.temperature && (
                        parseFloat(formData.temperature) >= 38.0 ? (
                          <span style={{ fontSize: 11, fontWeight: 700, color: '#b91c1c', background: '#fee2e2', padding: '1px 6px', borderRadius: 4 }}>High Fever</span>
                        ) : parseFloat(formData.temperature) >= 37.5 ? (
                          <span style={{ fontSize: 11, fontWeight: 700, color: '#b45309', background: '#fef3c7', padding: '1px 6px', borderRadius: 4 }}>Low Grade</span>
                        ) : (
                          <span style={{ fontSize: 11, fontWeight: 700, color: '#15803d', background: '#dcfce7', padding: '1px 6px', borderRadius: 4 }}>Afebrile / Normal</span>
                        )
                      )}
                    </div>
                  </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <div>
                    ...
                    <strong style={{ fontSize: 14, color: '#1e293b' }}>{formData.blood_pressure || '—'} mmHg</strong>
                  </div>
                </div>
              </div>
            ...
            <div>
              ...
              >
                <option value="">Select Doctor Re-Exposure Plan…</option>
                <option value="two_dose_booster">Two boosters — Day 0 &amp; Day 3 (Standard WHO/DOH Re-exposure)</option>
                <option value="single_booster">One booster — Day 0 only (Re-exposure &lt; 3 months)</option>
                <option value="full_pep">Start full PEP — Day 0, Day 3, Day 7 (Incomplete/uncertain prior history)</option>
                <option value="continue_existing_schedule">Continue existing prescribed schedule</option>
                <option value="no_vaccine">No additional rabies vaccine indicated (Wound care / TT only)</option>
              </select>
            </div>
          </div>
        )}
