  ...
  return (
    <div className="db-explorer-container">
      ...
      <div className="db-kpi-grid">
        ...
        <div className="db-kpi-card">
          <div className="db-kpi-header">
            <span className="db-kpi-label">Total Attributes</span>
        ...
        <div className="db-kpi-card">
          <div className="db-kpi-header">
            <span className="db-kpi-label">Total Indexed Rows</span>
        ...
        <div className="db-kpi-card">
          <div className="db-kpi-header">
            <span className="db-kpi-label">Privacy Shield</span>
            <Icon name="check" size={16} color="#17653a" />
          </div>
          <div className="db-kpi-value" style={{ color: '#17653a' }}>
            Protected
          </div>
          <div className="db-kpi-sub">Raw Data Hidden from API</div>
        </div>
      </div>
      ...
      >
        <button
          ...
          style={{
            ...
            color: activeTab === 'overview' ? '#065f46' : '#64748b',
            cursor: 'pointer',
            display: 'flex',
            alignItems: 'center',
            gap: 6,
        ...
        >
          <Icon name="reports" size={16} color={activeTab === 'overview' ? '#065f46' : '#64748b'} />
          All Database Tables Summary ({tables.length})
        </button>
        ...
        <button
          ...
          style={{
            background: 'none',
            border: 'none',
            borderBottom: activeTab === 'structure' ? '3px solid #10b981' : '3px solid transparent',
            padding: '8px 16px',
            fontSize: 14,
            fontWeight: activeTab === 'structure' ? 700 : 500,
            color: activeTab === 'structure' ? '#065f46' : '#64748b',
            cursor: 'pointer',
            display: 'flex',
            alignItems: 'center',
            gap: 6,
        ...
        >
          <Icon name="settings" size={16} color={activeTab === 'structure' ? '#065f46' : '#64748b'} />
          Table Structure & Attribute Inspector ({selectedTable || 'Select Table'})
        </button>
      </div>

      {/* TAB 1: ALL TABLES OVERVIEW (XAMPP phpMyAdmin db_structure style) */}
      {activeTab === 'overview' && (
        <div style={{ background: '#ffffff', border: '1px solid #e0eae3', borderRadius: 12, padding: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <div>
              <h2 style={{ fontSize: 16, fontWeight: 700, margin: 0, color: '#111827' }}>
                XAMPP MariaDB Tables Overview — <span style={{ color: '#059669', fontFamily: 'monospace' }}>{databaseName}</span>
              </h2>
              <p style={{ fontSize: 12.5, color: '#6b7280', margin: '2px 0 0' }}>
                Summary listing of all tables, primary keys, row counts, and attribute counts.
              </p>
            </div>
        ...
        <div className="db-explorer-split">
          ...
          <div className="db-explorer-sidebar">
            ...
            <div className="db-explorer-table-list">
              {loadingTables ? (
                <p style={{ padding: '1rem', fontSize: '0.8125rem', color: '#77877d' }}>Loading tables...</p>
              ) : (
                filteredTables.map((t) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="db-explorer-badge-count">{t.row_count}</span>
                    </button>
                  );
                })
              )}
          ...
          {/* Right Panel: Column Attributes & Data Types */}
          <div className="db-explorer-main">
            {selectedTable ? (
              <>
                ...
                >
                  ...
                  <div style={{ fontSize: '0.8125rem', color: '#475569' }}>
                    <span style={{ fontWeight: 600, color: '#173d29' }}>MariaDB Attribute Inspector: </span>
                    Showing data types, index keys, defaults, and nullability for table <code style={{ color: '#059669', fontWeight: 700 }}>{selectedTable}</code>.
                  </div>
                </div>
                ...
                <div className="db-explorer-toolbar">
                  <div>
                    <h2 className="db-explorer-table-title">
                      Attributes & Data Types: <span style={{ color: '#17653a', fontWeight: 700 }}>{selectedTable}</span>
                    </h2>
                    <span className="db-explorer-table-meta">
                      Total Records: {totalRecords} | Table Attributes: {columns.length}
                    </span>
                  </div>
                ...
                {/* Table Column Attributes View */}
                {loadingDetails ? (
                  <p style={{ padding: '2rem', textAlign: 'center', color: '#17653a' }}>Loading column structure...</p>
                ) : (
                  <div className="db-explorer-table-wrapper">
                    <table className="db-explorer-table">
                      <thead>
                        ...
                        {filteredColumns.map((col, idx) => (
                          <tr key={col.column_name}>
                            ...
                            <td style={{ fontWeight: 700, fontFamily: 'monospace', color: '#1e293b' }}>
                              {col.column_key === 'PRI' && (
                                <span title="Primary Key" style={{ marginRight: 4, display: 'inline-flex', verticalAlign: 'middle' }}>
                                  <Icon name="key" size={13} color="#059669" />
                                </span>
                              )}
                              {col.column_key === 'UNI' && (
                                <span title="Unique Key" style={{ marginRight: 4, display: 'inline-flex', verticalAlign: 'middle' }}>
                                  <Icon name="security" size={13} color="#d97706" />
                                </span>
                              )}
                              {col.column_name}
                            </td>
                            ...
                            <td>
                              <span
                                style={{
                                  background: col.is_nullable === 'YES' ? '#e8f5ed' : '#fee2e2',
                                  color: col.is_nullable === 'YES' ? '#17653a' : '#991b1b',
                            ...
                            <td style={{ fontWeight: 600, color: col.column_key === 'PRI' ? '#059669' : '#0d9488' }}>
                              {col.column_key ? (
                                <span style={{ bgcolor: '#f0fdf4', padding: '2px 6px', borderRadius: 4, fontSize: 11 }}>
                                  {col.column_key === 'PRI' ? 'PRIMARY (PRI)' : col.column_key === 'UNI' ? 'UNIQUE (UNI)' : col.column_key}
                                </span>
                              ) : (
                                '-'
                              )}
                            ...
                            <td style={{ color: '#475569', fontFamily: 'monospace', fontSize: 12 }}>
                              {col.column_default !== null ? col.column_default : <em>NULL</em>}
                            </td>
              ...
              <p style={{ color: '#77877d' }}>Select a table from the left list to view structure.</p>
            )}
          </div>
        </div>
      )}
