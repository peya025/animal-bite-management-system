...
- [x] **Migrate all card strings to AppLocalizations (context.tr) (en/tl/ceb)**
  ...
  - *Fix*: Use context.tr('guide_wash_title') namespaced across en.dart, fil.dart, and ceb.dart.

- [x] **Test ceb (Cebuano) and fil (Tagalog) string lengths against maxLines: 2 card bounds**
...
- [x] **Review Cebuano (ceb) phrasing with native speakers for clinical accuracy**
...
- [x] **Refactor inline magic numbers to consistent spacing scale (4/8/12/16/24)**
...
- [x] **Migrate widget-local guideTealBg and text grays into shared AppColors**
...
- [x] **Confirm all guide assets are 100% bundled locally for offline fortitude**
...
- [x] **Ensure fail-safe fallbacks exist for both illustrations and texts**
...
- [x] **Smallest Screen Widths**: Test on iPhone SE (1st/2nd gen, 320px-375px width) and small Android devices to confirm cards render without horizontal/overflow crush.
- [x] **System Font Scaling**: Test with largest accessibility font size enabled in OS settings (2.0x text scaler) to confirm text does not spill out of cards.
- [x] **Multi-Locale Validation**: Test English (en), Tagalog (fil), and Cebuano (ceb) using real approved strings and verify no unintended truncation.
- [x] **Contrast Checker Verification**: Run WCAG AA contrast checker against all text/background pairs and document passing design values (>= 4.5:1).
- [x] **Screen Reader Audit**: Test with TalkBack (Android) and VoiceOver (iOS); verify all 3 steps are announced with portrait descriptions.
- [x] **Offline Mode Survival**: Place device in Airplane mode with Wi-Fi/cellular disabled, launch app, and confirm cards display immediately.
